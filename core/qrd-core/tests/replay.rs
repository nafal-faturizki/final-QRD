...
fn replay_detects_duplicate_nonce() {
    ...
    assert_ne!(
        ...
        "Nonces should differ for different encryptions"
    );
}
...
fn replay_encryption_key_derivation_prevents_tampering() {
    ...
    assert_ne!(
        config1.column_name, config2.column_name,
...
fn replay_column_isolation() {
    ...
    assert_ne!(
        encrypted_col_a.ciphertext, encrypted_col_b.ciphertext,
        "Different payloads should produce different ciphertexts"
    );
}
...
fn replay_authentication_tag_prevents_modification() {
    ...
    assert_ne!(
        modified_ciphertext, encrypted.ciphertext,
        "Modified ciphertext should differ"
    );
}
...
fn replay_nonce_prevents_ciphertext_reuse() {
    ...
    let all_same_nonce = encrypted_samples
        .iter()
        .all(|e| e.nonce == encrypted_samples[0].nonce);
...
fn replay_multiple_encryption_calls_differ() {
    ...
    assert!(
        !all_same_nonce,
        "Nonces should vary across different encryptions"
    );
}
