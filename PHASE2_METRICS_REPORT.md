...
### Writer Hot Path Benchmark

Criterion measurements for the newly added writer benchmarks on the current workspace build:

| Operation | Time | Notes |
|-----------|------|-------|
| `writer_write_row_group_small` | ~248-277 ns | Serializes a small row group and caches it in the streaming writer |
| `writer_finish_small` | ~436-470 ns | Finalizes a writer with one row group into a canonical file image |

**Assessment:**
- ✅ Writer hot path is now directly measurable in Criterion
- ✅ The direct row-group/file-image path stays sub-microsecond for the small benchmark input
- ✅ This gives a stable baseline for later streaming/file-image refinements
